...
import ObjInterface from '../../../Domain/Entities/Item/Interface'
import Serviceable from '../../../Domain/Entities/Item/Ports/Serviceable'
...
export default class Controller implements Routeable, Patheable {
	...
	@inject(TYPES.Validable) @named(TYPES.Item) private dto: Validable
	@inject(TYPES.Schemable) @named(TYPES.Item) private schema: Schemable
	@inject(TYPES.ItemServiceableDomain) private service: Serviceable
	...
	private getAllObjs = async (request: RequestWithUser, response: Response, next: NextFunction) => {
		...
		recivedMatch.operationType = { $ne: "D" }
