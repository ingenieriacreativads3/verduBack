...
class WSFEV1 {
	...
	*/
	...
	public function __construct($build, $path, $database, $CUIT)
	{
		...
		if($build === "test") {
			if (!file_exists($this->WSDL)) $this->error .= " Failed to open ".$this->WSDL;
		}
	...
	private function _checkErrors($results, $method)
	{
		...
		file_put_contents($this->pathLogs, date("d/m/Y H:i:s") ." - WSFE _checkErrors: ". $method."\n", FILE_APPEND | LOCK_EX);
		file_put_contents($this->pathLogs, date("d/m/Y H:i:s") ." - WSFE _checkErrors result: ". json_encode($results)."\n", FILE_APPEND | LOCK_EX);
		...
		if ($method == 'FECAESolicitar') {
			if(sizeof($results->$XXX->Errors->Err) > 1) {
				$this->ObsCode = $results->$XXX->Errors->Err[0]->Code;
				$this->ObsMsg = $results->$XXX->Errors->Err[0]->Msg;
			} else if($results->$XXX->Errors->Err) {
				$this->ObsCode = $results->$XXX->Errors->Err->Code;
				$this->ObsMsg = $results->$XXX->Errors->Err->Msg;
			} else if (sizeof($results->$XXX->FeDetResp->FECAEDetResponse->Observaciones->Obs) > 1) {	
	...
	public function FECompUltimoAutorizado($ptovta, $tipo_cbte)
	{
		...
		file_put_contents($this->pathLogs, date("d/m/Y H:i:s") ." - WSFE ptovta: ". $ptovta."\n", FILE_APPEND | LOCK_EX);
		file_put_contents($this->pathLogs, date("d/m/Y H:i:s") ." - WSFE tipo_cbte: ". $tipo_cbte."\n", FILE_APPEND | LOCK_EX);
		file_put_contents($this->pathLogs, date("d/m/Y H:i:s") ." - WSFE CUIT: ". $this->CUIT."\n", FILE_APPEND | LOCK_EX);
		
		$params = array('Auth'=>array(	'Token' => $this->TA->credentials->token,
										'Sign' => $this->TA->credentials->sign,
										'Cuit' => $this->CUIT),
										'PtoVta' => $ptovta,
										'CbteTipo' => $tipo_cbte);
		file_put_contents($this->pathLogs, date("d/m/Y H:i:s") ." - WSFE params: ". json_encode($params)."\n", FILE_APPEND | LOCK_EX);
