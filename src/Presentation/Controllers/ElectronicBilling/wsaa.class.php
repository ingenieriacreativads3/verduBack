...
class WSAA {
  	...
  	public function __construct($build, $path, $database, $service = 'wsfe') 
	{
		...
		if(!empty($this->err)) {
			$err ='{
						"status":"err",
						"message":"WSAA class. Faltan archivos necesarios para el funcionamiento1. "'.$this->err.'
					}';
	...
	*/
	...
	public function get_expiration() 
	{
		...
		if(empty($this->TA)) {
			...
			file_put_contents("log.txt", date("d/m/Y H:i:s") . " ta: " . $this->path . $this->database . "/" . self::TA . "\n", FILE_APPEND | LOCK_EX);	
			if($TA_file) {
				$TA_xml = '';
		...
		file_put_contents("log.txt", date("d/m/Y H:i:s") . " asd: " .$r . "\n", FILE_APPEND | LOCK_EX);	
