...
import ObjInterface from '../../../Domain/Entities/Company/Interface'
import Serviceable from '../../../Domain/Entities/Company/Ports/Serviceable'
...
export default class Controller implements Routeable, Patheable {
	...
	@inject(TYPES.Validable) @named(TYPES.Company) private dto: Validable
	@inject(TYPES.Schemable) @named(TYPES.Company) private schema: Schemable
	@inject(TYPES.CompanyServiceableDomain) private service: Serviceable
	...
	private saveObj = async (request: RequestWithUser, response: Response, next: NextFunction) => {
		...
		await this.service.getAll(model, {}, { name: objData.name, operationType: { $ne: 'D' } }, {}, {}, 0, 0)
